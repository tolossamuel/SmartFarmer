...
class AuthenticationSystem:
    def register(self, password: str, email:str, name: str, country: str):
        ...
        try:
            ...
            if conn is None:
                ...
                if conn:
                    conn.close()
                return json.dumps({"success": False, "message": "Database connection failed."})
            ...
            if existing_user:
                ...
                if conn:
                    cursor.close()
                    conn.close()
                return json.dumps({"success": False, "message": "User with this email already exists."})
            ...
            if conn:
                cursor.close()
                conn.close()
            return json.dumps({"success": False, "message": "try again later!"})
        ...
        try:
            ...
            if user is None:
                ...
                if conn:
                    cursor.close()
                    conn.close()
                return json.dumps({"success": False, "message": "email or password is incorrect."})
    ...
    def updatePassword(self, userId: str, old_password: str, new_password: str):
        try:
            ...
            """, (new_hashed_password.decode('utf-8'), userId))

            conn.commit()
            if conn:
                cursor.close()
                conn.close()
            return json.dumps({"success": True, "message": "Password updated successfully."})
        ...
        try:
            ...
            if conn is None:
                ...
                if conn:
                    conn.close()
                return json.dumps({"success": False, "message": "Try again later!"})
            if not name or not email or not userId or not country:
                if conn:
                    conn.close()
                return json.dumps({"success": False, "message": "try again later!"})
            ...
            if conn:
                cursor.close()
                conn.close()
            return json.dumps({"success": True, "message": "User information updated successfully."})
    ...
    def getAllUsers(self):
        try:
            ...
            if conn:
                cursor.close()
                conn.close()
            return json.dumps({"success": True, "users": user_list})
